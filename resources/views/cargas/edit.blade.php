            ...
            @if ($carga->operacion->insumo->tipo === 'Consumible')
              <div class="col-sm-4">
                <div class="form-group">
                  ...
                  <input class="form-control" min="{{ today()->addDay()->format('Y-m-d') }}" value="{{ old('vencimiento', $carga->vencimiento?->format('Y-m-d')) }}" name="vencimiento" id="vencimiento" type="date" required />
                  @error('vencimiento')
                    <p class="text-danger">
                      {{ $message }}
                    </p>
